...
import {useAddFavoriteBookMutation} from '../../slices/books';
...
const DetailBook = ({
    ...
    const [addFavoritBook] = useAddFavoriteBookMutation();
    const [favorite, setFavorite] = useState(likedListUser?.length !== 0 ? 'heart-fill' : 'heart');

    const handleFavorite = async () => {
        const res = await addFavoritBook(id).unwrap();
        if (res) setFavorite(pre => (pre === 'heart' ? 'heart-fill' : 'heart'));
    };
    return (
        <View style={[styles.container, style]}>
            ...
            <View style={styles.bookInfo}>
                <View style={styles.titleContent}>
                    ...
                    <Octicons name={favorite} color="pink" size={22} onPress={handleFavorite} />
                </View>
