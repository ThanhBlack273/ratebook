...
const reviewsApi = rateBookApiSlice.injectEndpoints({
    endpoints: builder => ({
        ...
        deleteComment: builder.mutation({
            ...
            async onQueryStarted(arg, {queryFulfilled, dispatch, getState}) {
                try {
                    await queryFulfilled;
                    dispatch(
                        bookApi.endpoints.getBookInfo.initiate((getState() as RootState).book.currDetailBook, {
                            forceRefetch: true,
                        }),
                    );
                    dispatch(
        ...
        addLikeReview: builder.mutation<{liked: boolean}, number>({
            query: id => ({
                url: `/action/review/${id}/add_like`,
                method: 'POST',
            }),
        }),
    }),
